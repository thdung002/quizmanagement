...
module.exports = {
    ...
    getQuizContent: function (req, res) {
        QuizContent.getQuizContent(req.params.idquiz, function (err, arrQ) {
            if (err)
                return res.json({result:"fail",message:"Invalid input"});
            else return res.json({result:"ok",message: "QuizContent get successfully!", data: arrQ,code: 20000});
        })
    },
