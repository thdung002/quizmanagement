        ...
        methods: {
            ...
            handleDownload() {
                ...
                import('@/vendor/Export2Excel').then(excel => {
                    const tHeader = ['DateCreated', 'Questionid', 'Content', 'CorrectAnswer(for filling)', 'CorrectAnswer(for single-choice, multiple-choice)','status(0 is activated, 1 is deleted)'];
                    const filterVal = ['CreatedAt', 'Question', 'Content', 'CorrectAnswer','IsCorrect', 'IsDeleted'];
                    const data = this.formatJson(filterVal);
                    excel.export_json_to_excel({
                        header: tHeader,
                        data,
                        filename: 'table-answer'
                    });
