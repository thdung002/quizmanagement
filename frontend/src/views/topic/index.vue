...
<script>
    import {GetTopic, CreateTopic, UpdateTopic, DeleteTopic} from '@/api/topic'
        ...
        methods: {
            ...
            createData() {
                this.$refs.dataForm.validate((valid) => {
                    if (valid) {
                        ...
                        CreateTopic(this.temp).then((response) => {
            ...
            updateData() {
                this.$refs['dataForm'].validate((valid) => {
                    if (valid) {
                        ...
                        UpdateTopic(tempData, this.temp.ID).then((response) => {
                            if (response.result === "fail")
                                this.$notify({
                                    title: 'Warning',
            ...
            handleDownload() {
                ...
                import('@/vendor/Export2Excel').then(excel => {
                    const tHeader = ['DateCreated', 'Content','status(0 is activated, 1 is deleted)'];
                    const filterVal = ['CreatedAt', 'Content', 'IsDeleted'];
                    const data = this.formatJson(filterVal);
                    excel.export_json_to_excel({
                        header: tHeader,
                        data,
                        filename: 'table-topic'
                    });
